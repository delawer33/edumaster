    ...
    *,
    ...
    if entity_type == EntityType.MODULE:
        if parent_module_id:
            ...
            raise ValueError("Для модуля требуется course_id или parent_module_id")
        model = Module
        
    elif entity_type == EntityType.LESSON:
        if not parent_module_id and not course_id \
        or parent_module_id and course_id:
            raise ValueError("Для урока требуется parent_module_id (ID модуля) или course_id (ID курса)")
        if parent_module_id:
...
async def recalculate_orders(
    ...
    modules = await session.scalars(
        select(Module)
        .where(Module.id.in_(ordered_ids))
    )
