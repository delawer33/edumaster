...
from app.db import get_async_db_session, CoursePurchase, User
...
async def process_payment_stub(
    ...
    stmt = select(CoursePurchase).where(CoursePurchase.user_id == user.id)
    ...
    if transaction.status == "success":
        cp = CoursePurchase(
            user_id=user.id, course_id=course.id, transaction_id=transaction.id
        )
