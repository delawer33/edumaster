...
class CoursePolicy:
        ...
        if user.role == UserRole.teacher:
            return or_(
                ...
                and_(
                    ...
                    Course.status.in_(
                        [ObjectStatus.draft, ObjectStatus.archived]
                    ),
                ),
            )
    ...
    @classmethod
    async def check_course_archived(cls, course: Course):
    ...
    @classmethod
    async def check_single_course_access(cls, course: Course, user: User):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Доступ отклонен"
        )

    @classmethod
    async def check_module_belongs_course(cls, module: Module, course: Course):
    ...
    @classmethod
    async def check_lesson_belongs_course(cls, lesson: Lesson, course: Course):
    ...
    @classmethod
    async def check_lesson_belongs_module(cls, lesson: Lesson, module: Module):
    ...
    async def check_resource_access(
        ...
        if action == "write":
            ...
            if (
                isinstance(resource, Module)
                and resource.status == ObjectStatus.archived
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
        ...
        if action == "read":
            ...
            if (
                isinstance(resource, Lesson)
                and resource.status != ObjectStatus.published
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
